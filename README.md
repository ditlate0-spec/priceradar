...
- **Автоподтяжка комиссий с бирж.** У Binance, Bybit, OKX **нет публичных эндпоинтов комиссий** — все требуют signed request с API-ключом пользователя. Реализовано **ручное обновление через UI** с версионированием через `effective_from`. Это соответствует духу ТЗ и не требует работы с секретами пользователя.
...
- **Двухслойная подсветка.** В ТЗ описаны два слоя (рамка — секунды, фон — минуты). Реализован один слой — цвет ячейки. Двухслойный вариант отложен.

- **Максимум 10 пар, 3 биржи.** По ТЗ MVP. Расширение (Gate, MEXC, KuCoin, P2P) — план на v2.0.

- **Тесты.** Не написаны. По ТЗ — минимальный набор для формул и порогов. План — добавить перед production.
...
- **Пики по разным порогам независимы.** Один и тот же момент может дать 4 пика (по `0.3`, `0.5`, `1.0`, `2.0`).
